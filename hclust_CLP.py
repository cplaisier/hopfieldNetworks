...
def idx2color(idx, cidx):
    ...
    colors = [cm(1.*i/len(tmp)) for i in range(len(tmp))]
    #colors = {0:'b', 1:'r', 2:'g', 3:'c', 4:'m', 5:'k', 6:'y'}
    markers = {0:'o',1:'s',2:'p',3:'d',4:'^',5:'v',6:'>',7:'.',8:'h',9:'<'} # *, x, +
    return [colors[idx],markers[idx%10]]
...
def scatter_plot(points, cidx, marker='o', size=50, alpha=1.0):
    ...
    for i in xrange(len(set(cidx))):
        scatter(points[cidx==i,0], points[cidx==i,1], color=idx2color(i,cidx)[0],
                marker=idx2color(i,cidx)[1], s=size, alpha=alpha)
...
def scatter_plot3d(ax, points, energies, cidx, marker='o', size=50, alpha=1.0):
    ...
    for i in xrange(len(set(cidx))):
        ax.scatter(points[cidx==i,0], points[cidx==i,1], energies[cidx==i],
                   c=idx2color(i,cidx)[0], marker=idx2color(i,cidx)[1], s=size, alpha=alpha)
...
def plot_landscape_subset(data, res=50, subset=[], prune=None):
    """Plot the energy landscape of the Hopfield network in different ways.
       data -- tuple (labels, genes, samples) from load_data()
       res -- resolution of mesh
       prune -- pruning threshold. If None no pruning is performed
    ...
    # Subset samples
    subNums = [i for i in range(len(data[0])) if data[0][i] in subset]
    print subNums
    samples_subset = array([samples[i] for i in subNums], dtype=float)
    #print len(samples_subset)

    model2D = PCA(n_components=2)
    model2D.fit(samples_subset)

    W = hopfield_train(samples_subset, prune)
    states = samples_subset.copy()
    trajectories2d = [model2D.transform(samples)]
    trajectories = [states]
    for i in xrange(10):
        states = hopfield_ask(W, states, n=1)
...
def main(args):
    ...
    """
    print "running..."
    subset = ['0_exp_AM_1000','0_exp_PM_1000','0_sta_AM_1000','0_exp_AM_1000','0_exp_PM_1000','0_sta_AM_1000','0_exp_AM_1000','0_exp_PM_1000','0_sta_AM_1000','0_sta_PM_1000','0_exp_AM_300','0_exp_PM_300','0_sta_AM_300','0_sta_PM_300','0_exp_AM_300','0_exp_PM_300','0_sta_AM_300','0_sta_PM_300','0_exp_AM_300','0_exp_PM_300','0_sta_AM_300','0_sta_PM_300','0_sta_PM_1000','0_sta_PM_1000']
    ...
    #plot_landscape_subset(data, subset=subset, prune = t)
